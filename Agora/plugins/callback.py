...
from Agora import YouTube, app
from Agora.core.call import Anon
from Agora.misc import SUDOERS, db
from Agora.utils import bot_sys_stats
from Agora.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from Agora.utils.formatters import seconds_to_min
from Agora.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from Agora.utils.stream.autoclear import auto_clean
from Agora.utils.thumbnails import gen_thumb
    ...
    if command == "Pause":
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Agora.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Agora.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Agora.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
